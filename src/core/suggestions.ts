  ...
  for (let i = 0; i < rawGames.length; i++) {
    ...
    for (let j = i + 1; j < rawGames.length; j++) {
      ...
      if (similarity >= 0.85 && similarity < 1.0) {
        ...
        if (similarity >= 0.95) {
          reason =
            'Substring match - likely same game with different edition name';
        } else if (similarity >= 0.9) {
          reason =
            'Very high similarity - likely same game with minor name variation';
        } else {
          reason =
            'High similarity - possibly same game or related (e.g., sequel)';
        }
...
export const saveMergeSuggestions = async (
  ...
  const output = {
    ...
    instructions: {
      howToUse:
        'Review these suggestions and add confirmed merges to data/overrides.json in the "forceMerge" array',
      example: {
  ...
  console.log(
    `📝 Saved ${suggestions.length} merge suggestions to ${outputPath}`
  );
};
