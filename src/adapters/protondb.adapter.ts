...
const PROTONDB_URL = 'https://www.protondb.com/api/v1/reports/summaries';

/**
 * Map ProtonDB tier string to our enum
 */
const mapProtonTier = (tier: string): ProtonTier => {
 ...
 */
const getCachePath = (cacheDir: string, steamAppId: number): string => {
  return path.join(cacheDir, `${steamAppId}.json`);
};

/**
 * Fetch ProtonDB compatibility data
 */
const fetchProtonDBData = async (
  steamAppId: number
...
const getCached = async (
  ...
  steamAppId: number
): Promise<ProtonInfo | null> => {
  try {
    const cachePath = getCachePath(cacheDir, steamAppId);
 ...
 */
const init = async (cacheDir: string): Promise<void> => {
  try {
...
const fetchCompatibility = async (
  ...
  // Check cache first
  const cached = await getCached(cacheDir, cacheDurationMs, steamAppId);
  if (cached) {
    return cached;
  }
  ...
  try {
    ...
    const protonData = await fetchProtonDBData(steamAppId);

    // Fetch Steam Deck verification status
    const deckStatus = await fetchSteamDeckStatus(steamAppId);
    ...
    const protonInfo: ProtonInfo = {
      tier: mapProtonTier(protonData.tier),
      steamDeck: deckStatus,
      confidence: parseFloat(protonData.confidence) || undefined,
      trendingTier: protonData.trendingTier
        ? mapProtonTier(protonData.trendingTier)
        : undefined,
      lastUpdated: new Date(),
    };

    // Cache the result
    await setCached(cacheDir, steamAppId, protonInfo);
...
/**
 * Create a ProtonDB adapter instance with specific cache configuration
 * Uses factory pattern since cache configuration needs to be maintained across calls
 */
export const createProtonDBAdapter = (
  cacheDir = '.cache/protondb',
  cacheDays = 30
) => {
  const cacheDurationMs = cacheDays * 24 * 60 * 60 * 1000;

  return {
    init: () => init(cacheDir),
    fetchCompatibility: (steamAppId: number) =>
      fetchCompatibility(cacheDir, cacheDurationMs, steamAppId),
    cleanCache: () => cleanCache(cacheDir, cacheDurationMs),
  };
};
